...
router.post("/", async (req, res) => {
  try {
    ...
    const filters =
      req.query.filters || (req.body && req.body.filters)
        ? req.body.filters
        : {};
    ...
    const filterDetailString =
      filterDetail.length > 0 ? "&$filter=" + filterDetail.join(" and ") : "";

    const fieldsList = req.query.fields || (req.body && req.body.fields);
    const fields =
      fieldsList !== null && fieldsList !== undefined
        ? "&$select=" + fieldsList.toString()
        : "";

    let urlRequest = `${tenant}/data/${entity}?$format=application/json;odata.metadata=none&cross-company=true&${fields}${
      testMode ? "$top=5" : ""
    }&$count=true`;
    axios
      .get(urlRequest, { headers: { Authorization: "Bearer " + token } })
      .then((response) => {
        const data = {
          entity,
          filters,
